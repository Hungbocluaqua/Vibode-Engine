...
struct ImGui_ImplVulkan_ViewportData
{
    ...
    ImVector<char>                          SwapchainImageHasPresentedLayout;
    bool                                    WindowOwned;
...
static void ImGui_ImplVulkan_ResetViewportImageLayouts(ImGui_ImplVulkan_ViewportData* vd)
{
    ImGui_ImplVulkanH_Window* wd = &vd->Window;
    vd->SwapchainImageHasPresentedLayout.resize((int)wd->ImageCount);
    if (vd->SwapchainImageHasPresentedLayout.Data != nullptr)
        memset(vd->SwapchainImageHasPresentedLayout.Data, 0, vd->SwapchainImageHasPresentedLayout.size_in_bytes());
}
...
void ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance instance, VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, uint32_t queue_family, const VkAllocationCallbacks* allocator, int width, int height, uint32_t min_image_count, VkImageUsageFlags image_usage)
{
    ...
    // Presentable images are owned by the presentation engine until acquired.
    // Do not pre-transition all images here; each image is transitioned after
    // vkAcquireNextImageKHR returns it in ImGui_ImplVulkan_RenderWindow().
}
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport)
{
    ...
    ImGui_ImplVulkan_ResetViewportImageLayouts(vd);
    vd->WindowOwned = true;
...
static void ImGui_ImplVulkan_RenderWindow(ImGuiViewport* viewport, void*)
{
    ...
    ImGui_ImplVulkanH_FrameSemaphores* render_fsd = nullptr;
    {
        ...
        if (v->UseDynamicRendering)
        {
            ...
            barrier.oldLayout = (wd->FrameIndex < (uint32_t)vd->SwapchainImageHasPresentedLayout.Size && vd->SwapchainImageHasPresentedLayout[(int)wd->FrameIndex]) ? VK_IMAGE_LAYOUT_PRESENT_SRC_KHR : VK_IMAGE_LAYOUT_UNDEFINED;
...

    {
        ...
        if (v->UseDynamicRendering)
        {
            ...
            if (wd->FrameIndex < (uint32_t)vd->SwapchainImageHasPresentedLayout.Size)
                vd->SwapchainImageHasPresentedLayout[(int)wd->FrameIndex] = 1;
        }
        else
#endif
        {
